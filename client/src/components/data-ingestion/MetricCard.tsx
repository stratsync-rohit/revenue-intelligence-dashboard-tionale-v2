...
const UpIcon: React.FC = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden width="12" height="12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M12 19V6" />
...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, change = "", style, className }) => {
  const raw = (change ?? "").trim();
  const lower = raw.toLowerCase();

  const isDuration = /\b\d+\s*(days?|d)\b/.test(lower) || /\b(days?|d)\b/.test(lower);
  const isPositive = raw.startsWith("+") || lower.includes(" up") || lower.includes("↑");
  const isNegative = raw.startsWith("-") && !isDuration;

  const toneClass = isNegative ? "badge-danger" : isPositive ? "badge-success" : "badge-neutral";
